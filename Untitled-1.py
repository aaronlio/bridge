...
class Bridge:
    ...
    def SFD_train(self):
        ...
        for i in range(self.length-856):
        
            reaction_a = 0
            for i in range(6):
                individual_a = (400/6)*((wheel_positions[i]-support_a_position))
                reaction_a += individual_a
                reaction_a = reaction_a/(1090-15)
        
        print(reaction_a)
        position = np.linspace(0, self.length, 1)
        self.shear_train = np.zeros((1, self.length))
        for i in range(self.length):
            if i in wheel_positions:
                self.shear_train
        self.shear[0][15:564] = .302
        self.shear[0][565:1059] = -.698
        self.shear[0][1060:1264] = 1

    def SFD_point_loads(self):
        position = np.linspace(0, self.length, 1)
        self.shear_train = np.zeros((1, self.length))
        self.shear_train[0][15:564] = .302
        self.shear_train[0][565:1059] = -.698
        self.shear_train[0][1060:1264] = 1

    def BMD_point_loads(self):
        position = np.linspace(0, self.length, 1)
